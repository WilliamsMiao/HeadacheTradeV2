...
数据异常、缺失、零成交量或 OHLC 不一致会写入 `KLine.data_ok=false`，流水线会冻结新入场建议。

## 指标

`app.services.indicators` 按时间顺序计算 MA20、MA60、MACD DIF、DEA、MACD 柱、ATR、成交量均线。指标表与原始行情表分离，指标模块不负责买卖判断。
